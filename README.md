...
```bash
npm install tool-validator zod
```
...
And they do it differently every time, for every framework, in every project. `tool-validator` is the standard.
...
```typescript
import { tool, registry } from 'tool-validator';
import { z } from 'zod';
...
```typescript
import { toOpenAI, toAnthropic, toGemini, toVercelAI } from 'tool-validator';
...
```typescript
import type { InferInput, InferOutput } from 'tool-validator';
...
`tool-validator` detects the Zod version automatically and uses the right conversion path.
...
import { registry, tool } from 'tool-validator';
import { z } from 'zod';
